...
var _ = Describe("Apply Apps", func() {
	...
	Describe("on updating/creating virt-handler", func() {
		...
		Context("updating virt-handler", func() {
			...
			createCanaryPod := func(kv *v1.KubeVirt, daemonSet *appsv1.DaemonSet, phase corev1.PodPhase, ready bool) *corev1.Pod {
				...
				pod := &corev1.Pod{
					ObjectMeta: v12.ObjectMeta{
						OwnerReferences: []v12.OwnerReference{
							{Name: daemonSet.Name, Controller: &boolTrue},
						},
			...
			createCrashedCanaryPod := func(kv *v1.KubeVirt, daemonSet *appsv1.DaemonSet) *corev1.Pod {
				pod := createCanaryPod(kv, daemonSet, corev1.PodRunning, false)
				pod.Status.ContainerStatuses[0].RestartCount = 1
				return pod
			}

			It("should just reconcile if virt-handler is up-to-date", func() {
				mockDSCacheStore.get = daemonSet
				SetGeneration(&kv.Status.Generations, daemonSet)
				patched := false
				...
				newDs.ObjectMeta.Annotations = map[string]string{
					"reconcile-me": "",
				}
			...
			type daemonSetBuilder func(*v1.KubeVirt, *appsv1.DaemonSet) (current *appsv1.DaemonSet, target *appsv1.DaemonSet, podInStore *corev1.Pod)
			...
			table.DescribeTable("process canary upgrade",
				func(dsBuild daemonSetBuilder,
					...
					currentDs, newDs, pod := dsBuild(kv, daemonSet)
					if pod != nil {
					...
					done, err, status := r.processCanaryUpgrade(currentDs, newDs)
				...
				table.Entry("should start canary upgrade with MaxUnavailable 1",
					func(kv *v1.KubeVirt, currentDs *appsv1.DaemonSet) (*appsv1.DaemonSet, *appsv1.DaemonSet, *corev1.Pod) {
						newDs := daemonSet.DeepCopy()
						addCustomTargetDeployment(kv, newDs)
						return currentDs, newDs, nil
					},
				...
				table.Entry("should wait for canary pod to be ready",
					func(kv *v1.KubeVirt, currentDs *appsv1.DaemonSet) (*appsv1.DaemonSet, *appsv1.DaemonSet, *corev1.Pod) {
						...
						addCustomTargetDeployment(kv, newDs)
						addCustomTargetDeployment(kv, currentDs)
						return currentDs, newDs, createCanaryPod(kv, daemonSet, corev1.PodPending, false)
					},
					func(kv *v1.KubeVirt, daemonSet *appsv1.DaemonSet) {},
					CanaryUpgradeStatusStarted, false, false, false,
				),
				table.Entry("should restart daemonset rollout with MaxUnavailable 10%",
					func(kv *v1.KubeVirt, currentDs *appsv1.DaemonSet) (*appsv1.DaemonSet, *appsv1.DaemonSet, *corev1.Pod) {
						...
						addCustomTargetDeployment(kv, newDs)
						addCustomTargetDeployment(kv, currentDs)
						currentDs.Status.DesiredNumberScheduled = 2
						currentDs.Status.NumberReady = 1
						currentDs.Spec.UpdateStrategy.RollingUpdate = &appsv1.RollingUpdateDaemonSet{
							MaxUnavailable: nil,
						}
						return currentDs, newDs, createCanaryPod(kv, daemonSet, corev1.PodRunning, true)
					},
					func(kv *v1.KubeVirt, daemonSet *appsv1.DaemonSet) {
						Expect(util.DaemonSetIsUpToDate(kv, daemonSet)).To(BeTrue())
						rollingUpdate := daemonSet.Spec.UpdateStrategy.RollingUpdate
				...
				table.Entry("should report an error when canary pod fails",
					func(kv *v1.KubeVirt, currentDs *appsv1.DaemonSet) (*appsv1.DaemonSet, *appsv1.DaemonSet, *corev1.Pod) {
						...
						addCustomTargetDeployment(kv, newDs)
						addCustomTargetDeployment(kv, currentDs)
						return currentDs, newDs, createCrashedCanaryPod(kv, daemonSet)
					},
					func(kv *v1.KubeVirt, daemonSet *appsv1.DaemonSet) {},
				...
				table.Entry("should complete rollout",
					func(kv *v1.KubeVirt, currentDs *appsv1.DaemonSet) (*appsv1.DaemonSet, *appsv1.DaemonSet, *corev1.Pod) {
						...
						addCustomTargetDeployment(kv, newDs)
						addCustomTargetDeployment(kv, currentDs)
						currentDs.Status.DesiredNumberScheduled = 1
						currentDs.Status.NumberReady = 1
						currentDs.Spec.UpdateStrategy.RollingUpdate = &appsv1.RollingUpdateDaemonSet{
							MaxUnavailable: &maxUnavailable,
						}
