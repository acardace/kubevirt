...
func (c *MigrationTargetController) syncVolumes(
	vmi *v1.VirtualMachineInstance,
	client cmdclient.LauncherClient) (map[string]*containerdisk.DiskInfo, error) {
	...
	if ready, err := c.containerDiskMounter.ContainerDisksReady(vmi, info.NotInitializedSince); !ready {
		...
		return nil, container_disk.ErrWaitingForDisks
	}
	...
	case goerror.Is(err, container_disk.ErrChecksumMismatch):
		...
		return nil, client.SignalTargetPodCleanup(vmi)
	case err != nil:
		return nil, err
	}
	...
	if attachmentPodUID := vmi.Status.MigrationState.TargetAttachmentPodUID; attachmentPodUID != types.UID("") {
		...
		if err := c.hotplugVolumeMounter.MountFromPod(vmi, attachmentPodUID, cgroupManager); err != nil {
			return nil, fmt.Errorf("failed to mount hotplug volumes: %v", err)
		}
	}
...
func (c *MigrationTargetController) processVMI(vmi *v1.VirtualMachineInstance) error {
	...
	disksInfo, err := c.syncVolumes(vmiCopy, client)
